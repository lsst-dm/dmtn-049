...
\addtohist{3}{2020-12-23}{Added section 3 Technical Considerations. Some other minor revisions. }{Leanne Guy}
}
...
The goal is to provide the community with some initial technical boundaries and expectations for their proposals. 
...
The DM team will ensure that all measured quantities needed as inputs to photo-$z$ estimators are computed and included in the {\tt Object} catalog (See \S~\ref{ssec:dp_objvals}). 

The outputs currently foreseen and budgeted for, irrespective of algorithm, are defined in the LSST Data Products Definition Document (DPDD) \cite{LSE-163}, and are presented in Section \ref{ssec:docs_dpdd}. 
Exact schema definitions for output user-facing data products are stored in the github repository sdm\_schemas (\url{https://github.com/lsst/sdm\_schemas}). 
...
Additional storage may be required for ancillary input data such as spectral templates, calibration data or training sets (Appendix \ref{ssec:dp_calib}). 
Storage for outputs defined in the the DPDD \cite{LSE-163} has already been budgeted for by DM and is presented in \S~\ref{ssec:dp_objvals}). 
...
Photo-$z$ estimators are expected to have a low-memory-footprint and to operate only on measurements in the LSST {\tt Object} catalog (Appendix \ref{ssec:dp_objvals}). 
