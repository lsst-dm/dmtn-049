...
The proposed minimum performance is likely achievable for LSST data with current photo-$z$ estimators, as demonstrated by, e.g., \citet{2018AJ....155....1G}, \textcolor{red}{Schmidt et al. 2020}.
...
\item \textcolor{red}{Schmidt et al. 2020} statistically compare the posterior probability distribution functions produced by 12 photo-$z$ estimators for a mock data set that is representative of the LSST, identifying some biases and shortfalls in both the produced PDFs and the evaluation methods used to analyze them.
\end{itemize}
...
Journal articles that demonstrate validation processes for photo-$z$ from multi-band wide-area surveys include {\it "DES science portal: Computing photometric redshifts"} \citep{2018A&C....25...58G}; {\it "Photometric redshifts for Hyper Suprime-Cam Subaru Strategic Program Data Release 1"} \citep{2018PASJ...70S...9T}; and {\it "On the realistic validation of photometric redshifts"} \citep{2017MNRAS.468.4323B}. Furthermore, DESC is currently investigating methods of validating accuracy of probability distributions from a photo-$z$ algorithm (see, e.g., \textcolor{red}{Schmidt et al. 2020 in prep.}).
...
\item the conditional density estimation (CDE) loss as described in Section 4.2 of \textcolor{red}{Schmidt et al. 2020}. (Note that the CDE does not strictly require the true posterior be known.)
\end{itemize}
