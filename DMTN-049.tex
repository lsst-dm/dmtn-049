...
\bigskip
This roadmap proposes three phases for the DM team:
\begin{itemize}
\item {\bf Phase 0:} Finalize this plan and its timeline (Dec 31 2020).
\item {\bf Phase 1:} Define the criteria for evaluating photo-$z$ estimators (Aug 31 2021) by soliciting community input regarding their science needs and interests.
\item {\bf Phase 2:} Prepare a ranked list of community-vetted photo-$z$ estimator(s) by soliciting community "letters of recommendation" (due Apr 31 2022).
\item {\bf Phase 3:} Make a final selection of the community-vetted photo-$z$ estimator(s) to be used for DR1 by facilitating community participation in a "photo-$z$ validation data challenge" based on commissioning data.
\end{itemize}
}
...
{\bf Oct, Nov 2020:} Hold two LSST Photo-$z$ Forums that focus on gathering project and community input on the proposed roadmap and its timeline. \\
...
\subsection{Phase 3: Photo-$z$ Validation Data Challenge}\label{ssec:time_pzdc}

{\bf During commissioning} the DM team will facilitate community participation in a "Photo-$z$ Validation Data Challenge" (\S~\ref{sec:pzdc}).
The DM team will make any final decisions regarding which photo-$z$ estimator(s) will be implemented for DR1 {\it after} this data challenge. 
The details and the timeline for this data challenge are to be determined, as they are contingent on the commissioning schedule.
The types of commissioning data that might be most useful for this data challenge are described in \S~\ref{ssec:pzdc_commissioning}.
A preliminary collection of photo-$z$ validation techniques are presented in Appendix \ref{sec:imp}.
Full documentation for the validation process and associated data challenge is considered beyond the scope of this document, and will appear elsewhere.

\subsection{Rubin Operations}\label{ssec:time_ops}

{\bf During operations} the Data Production team will produce and validate photo-$z$ for the {\tt Object} catalog using the software and data products delivered by the Construction-era DMS (which may include in-kind contributions, described in Appendix \ref{ssec:opts_inkind}).
...
\begin{figure}
\begin{center}
\includegraphics[width=17cm]{timeline.pdf}
...
The criteria for evaluating performance overlaps with the validation tests discussed in Appendix \ref{ssec:imp_val}, and evaluating the performance of potential photo-$z$ estimators with LSST data will be part of the validation data challenge  based on commissioning data (Phase 3; \S~\ref{sec:lor}).
...
The DM team's goal is to create a shortlist of several community-vetted photo-$z$ estimator(s) that will serve as broad a range of LSST science goals as possible and also meet any internal requirements.
The shortlisted estimators would then be included in the "Photo-$z$ Validation Data Challenge" based on commissioning data (\S~\ref{sec:pzdcs}). 
...
\textit{TBD: this section will describe which community-vetted photo-$z$ estimator(s) DM has shortlisted to move on to Phase 3, and why.}


\clearpage
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\section{Proposed Photo-$z$ Validation Data Challenge}\label{sec:pzdc}
...
Towards this end, the DM team plans to facilitate a photo-$z$ validation data challenge based on LSST commissioning data.
This data challenge should not be seen as a competition, but rather a collaborative endeavor between project and community to maximize the scientific utility of the {\tt Object} catalog photo-$z$ data products.
{\bf It is anticipated that this process will start with open communication about what the science community needs, in terms of commissioning data and access to tools and computational resources, to fully participate in the data challenge.}
A full description of this data challenge, and the implementation and validation process, is considered beyond the scope of this document and will appear elsewhere.

\subsection{Potential Commissioning Data for Photo-$z$ Validation}\label{ssec:pzdc_commissioning}
...
DP2 is more likely to provide the data set for the photo-$z$ validation data challenge, and also some of the needed training data for LSST photo-$z$ described in Appendix \ref{ssec:dp_calib}.
...
The process outlined by this document for Data Management to select, implement, and validate a photo-$z$ estimator in the Data Release pipeline, with input from the science community, would also apply to any international team providing a photo-$z$ software package (or related data products) for the {\tt Object} catalog as an in-kind contribution in exchange for data rights. 
Such teams should expect to work closely with Data Management on each step of the proposed process in \S~\ref{sec:time}, including the solicitation of community input, the process of implementation and validation, and maintaining and evolving the photo-$z$ estimator during Rubin Operations
In-kind contributors should also review the summary of related data products and documentation that they would be expected to assemble and maintain (Appendice \ref{sec:dp} and  \ref{sec:docs}), and the example scientific use-cases that the photo-$z$ estimates are expected to support (Appendix \ref{sec:use}).
