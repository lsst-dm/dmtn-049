...
\textbf{The primary milestone in this roadmap is DM's call for "Letters of Recommendation" from the science community regarding photo-$z$ estimators (\S~\ref{sec:lor}; preliminary deadline 2021-10-01).}
...
\medskip
In support of this milestone, starting in early 2021, DM will host a series of "LSST Photo-z Virtual Forums", discussions open to all interested people, which will focus on defining the evaluation criteria for photo-$z$ estimators (\S~\ref{sec:eval}).

\medskip
As part of DM's process to select, implement, and validate one or more estimator(s) from the shortlist, DM will facilitate science community participation in a "Photo-$z$ Validation Cooperative" based on commissioning data (\S~\ref{sec:pzcoop}; dates TBD). 
...
\addtohist{4}{2020-12-23}{Added Section 3:Technical Considerations, revised Section C4:Early Science and Section 6:Commissioning. Some other minor revisions. }{Leanne Guy}
}
...
% DM (i.e., not part of DM's specialized knowledge base), 
...
% At all stages of this photo-$z$ roadmap, in-kind program contributions r
...
\textbf{Training and Iterative Development:}
Photo-$z$ estimators often require training of some kind before use on a particular dataset, or even a particular reprocessing of a particular dataset, in order to accurately relate the detailed properties and tendencies of the input photometric measurements to external data.
This is rarely something that can be fully automated.
Our ideal scenario is a ``pre-trained'' estimator that can be run at scale on the full {\tt Object} catalog with no human intervention.
This would typically include both software and auxilliary data that represents (and probably condenses) the results of any training procedure.

To construct such an estimator, we understand that it will probably be necessary to actually train it with photometry that is representative of the target data release, on fields that include at least some of the spectroscopic or external many-band photometry the estimator is trained against.
We plan to perform preliminary processing of small areas of sky prior to beginning full data release processing in earnest, in order to validate the software to be used in that processing.
We expect to make the photometry from this processing available to photo-$z$ collaborators to provide an opportunity for interactive training and tuning before the estimator and its auxiliary data are finalized.
...
While the needs of the photo-$z$ estimator will play a role in what preliminary processing is performed, these will need to be balanced with time constraints, our ability to use that processing for validation, and possibly data rights considerations as well.
...
Photo-$z$ estimators that cannot work well under these conditions may of course still be extremely valuable for LSST science, but it probably makes more sense to run these after a data release has gone out, and federate them with it later, to provide enough time for more complete training and validation.

If DM is unable to identify an algorithm (and collaborators) for which fast-turnaround, limited-input training is viable, we may simply run an off-the-shelf algorithm with no training at all, to serve as a placeholder until a better post-DR, community-built federated catalog becomes available.
...
% LSE-79 is very out of date and needs updating before we reference it here. 
...
% and might include, e.g., tests of the scheduler, tests of image quality, single-visit depth, astrometry, and photometry.
...
% JIRA ticket DM-6367
There is a requirement\dmreq{0046} on the Data Management System (DMS) that states that {\it "The DMS shall compute a photometric redshift for all detected Objects"} \citedsp{LSE-61}. 
