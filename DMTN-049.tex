...
\medskip
The DR {\tt Object} catalog photo-$z$ estimates will initially be a {\it minimum scientifically-viable product} that serve the {\it widest variety of science applications}.
DM will select one or more {\it existing, community-vetted algorithms} to generate the DR photo-$z$.
The selection criteria will include technical considerations related to the DM System. 
...
Therefore, the selection, implementation, and validation of the DR {\tt Objects} photo-$z$ estimator(s) will be a joint venture between the Rubin construction team and the Rubin science community. 
...
\textbf{The primary milestone in this roadmap is DM's call for "Letters of Recommendation" from the science community regarding photo-$z$ estimators (\S~\ref{sec:lor}; provisional deadline Oct 1 2021).}
...
\medskip
In support of this milestone, starting in early 2021 DM will host a series of "LSST Photo-z Virtual Forums", discussions open to all interested people, which will focus on defining the initial minimum scientific attributes and selection criteria for photo-$z$ estimators (\S~\ref{sec:mvp} \& \ref{sec:eval}).
As part of DM's process to select, implement, and validate one or more estimator(s) from this shortlist, DM will facilitate science community participation in a "Photo-$z$ Validation Cooperative" based on commissioning data (\S~\ref{sec:pzcoop}). 

\medskip
This roadmap includes in-kind contributions from international partnerships to {\tt Object} catalog photo-$z$ for DR1 and beyond, as well as potential processes of improving the DR {\tt Object} catalog photo-$z$ and/or federating user-generated photo-$z$ catalogs during Rubin operations.
}
...
All items in this timeline reference actions taken by DM to engage the science community.

{\bf Jan 1 2021:} Write a summary of this proposed timeline and advertise it broadly.

{\bf Jan -- Apr 2021:} Hold four LSST Photo-$z$ Virtual Forums, one per month, which focus on ingesting input from the science community regarding the proposed minimum scientific attributes and evaluation criteria (\S~\ref{sec:mvp} and \S~\ref{sec:eval}). Advertise the forums broadly.

{\bf May 1 2021:} Write a call for "Letters of Recommendation" from the science community which includes a summary of the minimum scientific attributes and evaluation criteria, and advertise it broadly .

{\bf May -- Aug 2021:} Hold four LSST Photo-$z$ Virtual Forums, one per month, which focus on the science community's efforts to prepare "Letters of Recommendation" that formally express support for certain photo-$z$ estimators (\S~\ref{sec:lor}). Advertise the forums broadly.

{\bf Oct 1 2021:} Deadline for "Letters of Recommendation" from the science community.

{\bf Before commissioning,} DM will generate a shortlist of community-vetted estimators to consider during commissioning, and add the rationale for these choices to \S~\ref{ssec:lor_choice}. Advertise this update broadly.

{\bf During commissioning,} the DM team will facilitate community participation in a "Photo-$z$ Validation Cooperative", which will analyze photo-$z$ estimates from the shortlisted estimators based on commissioning data (\S~\ref{sec:pzcoop}).
...
$\bullet$ {\bf Ingesting input from the science community} means that this document will be updated to include contributions from the science community to (e.g., updating the definitions of the initial minimum scientific attributes in \S~\ref{sec:mvp}).
...
\subsection{Evolving DR Object Photo-$z$ During Operations}\label{ssec:time_ops}

During operations, the Data Production team will generate photo-$z$ for {\tt Object} catalogs using the software and data products delivered by the Construction-era DMS, and make available any and all supporting materials such as documentation or spectral templates.
...
\subsection{Option to Federate a User-Generated Photo-$z$ Catalog}\label{ssec:time_ops_ugfed}
...
At all stages of this photo-$z$ roadmap, in-kind program contributions related to photo-$z$ should be included and integrated into the communal efforts to optimize the LSST photo-$z$. 
...
\clearpage
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\section{Technical Directives from the DM System}\label{sec:dmcon}

The following is a \textbf{\textit{preliminary draft}} set of {\it initial} technical directives for potential photo-$z$ estimators to be implemented in the DM System.
Community input on the feasibility or challenges of these directives is solicited, and this topic will be open for discussion during the LSST Photo-$z$ Virtual Forum series.
The DM team endeavors to be clear about the rigidity of these technical directives, but the following is subject to change as the DM System evolves.
Compliance with these constraints is one of the proposed evaluation criteria in \S~\ref{sec:eval}.

\textit{This is where DM must write down the constraints on the photo-$z$ estimators, in order to provide the community with some boundaries and expectations. Here are some examples.}
