...
This is a \textit{\textbf{living document}} that will evolve over time to incorporate input from the science community (including the Science Collaborations), the Rubin Observatory Data Management and Operations teams, and potential in-kind contributors.
This roadmap proposes two phases: (1) soliciting community input to define the criteria for evaluating photo-$z$ estimator(s), and (2) soliciting community white papers that advocate for/against potential photo-$z$ estimators. 
{\bf The endpoint of this roadmap is when the Data Management team chooses one or more community-vetted estimator(s) to be implemented into the Data Release pipeline.}
}
...
This roadmap guides the Rubin Observatory Data Management (DM) team towards a community-informed decision on which photo-$z$ estimator(s) will be implemented into the DR pipeline and served for the {\tt Object} catalog.
...
Below, the term ``advertise broadly" generally means posting to Community.lsst.org, sending emails to Science Collaborations and other lists of potentially interested individuals (e.g., authors of papers containing the terms LSST and photo-$z$), and including in Rubin newsletters. 
...
It is not intended that all interested people attend all forums.
The timing of the forums will be varied so as to enable participation from all timezones in at least a couple of forums.
...
{\bf Dec 1 2020:} Write a solicitation for feedback on the minimum attributes (\S~\ref{sec:mvp}) and evaluation criteria (\S~\ref{sec:sel}) and advertise it broadly. \\
...
{\bf May 31 2021:} Synthesize feedback and finalize \S~\ref{sec:mvp} and \S~\ref{sec:sel} of this document.

\subsection{Jan 31 2022: Receive White Papers from the Science Community}\label{ssec:time_wp}

{\bf June 1 2021:} Write a call for scientific white papers to evaluate the relative performance of various photo-$z$ estimators and demonstrate science impacts, and advertise it broadly.
...
{\bf Jan 31 2022:} Deadline for science white papers submitted to Data Management.
...
The rational behind this selection will be added to this document. 
...
Preliminary guidelines for this process are collected in Appendix \ref{sec:imp} in order to inform the evaluation criteria (\S~\ref{sec:sel}).
In particular, Appendices \ref{ssec:imp_community} and \ref{ssec:imp_commissioning} provide some preliminary information about incorporating community efforts, and the potential role of commissioning data, in the photo-$z$ validation process.
However, the implementation and validation process for DR photo-$z$ is beyond the scope of this document, and will recorded elsewhere.
...
\textbf{These proposed evaluation criteria will be refined via an iterative process between the Data Management team and the scientific community.} The supporting material in Appendices \ref{sec:imp}, \ref{sec:dp}, and \ref{sec:use} are also open for community input during the roadmap phase in \S~\ref{ssec:time_mvp}.
...
These criteria may also apply to community-generated catalogs proposing to be federated with the {\tt Objects} table (or be otherwise served at scale to the broader science community; \S~\ref{ssec:opts_ugfed}).
...
Note that as described in \S~\ref{ssec:imp_commissioning}, since the timing and suitability of the data previews during commissioning is currently unclear, the science community's contributions to the photo-$z$ evaluation and selection process is likely to rely on simulated LSST data or data from other facilities.
