...
A brief summary of the commonalities between science use-cases for photo-$z$ is provided in \S~\ref{sssec:use_sci_sum}.
...
The following is a proposed 6-step path for DM to generate {\tt Object} catalog photo-$z$ that meet {\it the basic} science needs for communities which will not or cannot generate custom photo-$z$, and also DM's internal use-cases, as described in \S~\ref{sec:use}.
...
DM runs validation tests to ensure that the {\tt Object} catalog photo-$z$ meet the defined MVP qualities, and to identify and characterize failure modes.
...
Diagnostics regarding the photo-$z$ results are made available (e.g., as auto-generated documentation) to inform the community's use of the {\tt Object} photo-$z$ (potential diagnostic tests are collected in \S~\ref{sec:diagnostics}).
...
Based on the use-cases for {\tt Object} photo-$z$ presented in \S~\ref{sec:use}, the following is proposed for the type, basic attributes, and minimum performance of the {\tt Object} photo-$z$.
...
Shortly before data release, the project provides the community team(s) with a "DR preview" so that they can train and calibrate their photo-$z$ algorithm.
...
These diagnostics would be based on the attributes/performance defined in \S~\ref{sec:mvp} and the evaluation criteria defined in \S~\ref{sec:sel}. 
...
In \S~\ref{ssec:sel_lit} we cover some existing work that compares photo-$z$ estimators. 
In \S~\ref{ssec:sel_ex} we apply a couple of of-the-shelf estimators to simulated LSST photometry to demonstrate a few statistical techniques for comparing photometric redshifts.
...
We have used simulated galaxy catalogs (\S~\ref{sssec:sel_ex_cats}) and three different photo-$z$ estimators (\S~\ref{sssec:sel_ex_estimators}) to generate photometric redshifts. We then apply a series of diagnostics to evaluate and compare their performance (\S~\ref{sssec:sel_ex_comp}).
